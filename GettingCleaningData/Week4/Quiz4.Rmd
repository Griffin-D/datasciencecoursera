...
```r
getwd()
library(data.table, quantmod, knitr)
setInternet2(TRUE)
```
Question 1
----------
...
head(variableNames)
variableNamesSplit <- strsplit(variableNames, "wgtp")
head(variableNamesSplit)
variableNamesSplit[[123]]
```
```
# playing
summary(variableNamesSplit)
str(variableNamesSplit)
```
...
f <- file.path("GDP.csv")
download.file(url, f)
GDPdata <- data.table(read.csv(f, skip = 4, nrows = 215, stringsAsFactors = FALSE))
head(GDPdata)
GDPdata <- GDPdata[X != ""]
head(GDPdata)
...
head(GDPdata)
str(GDPdata)
gdp <- as.numeric(gsub(",", "", GDPdata$gdp, fixed = TRUE))
head(gdp)
mean(gdp, na.rm = TRUE)
```
...
```r
beginsWithUnited <- grepl("^United", GDPdata$CountryName)
summary(beginsWithUnited)
```
...
download.file(url, f)
EdStats <- data.table(read.csv(f))
head(EdStats)
MergedDataTables <- merge(GDPdata, EdStats, all = TRUE, by = c("CountryCode"))
head(MergedDataTables)
FiscalYear <- grepl("fiscal year", tolower(MergedDataTables$Special.Notes))
June <- grepl("june", tolower(MergedDataTables$Special.Notes))
table(FiscalYear, June)
```

```
Results
          June
        FiscalYear FALSE TRUE
            FALSE   202    3
            TRUE     20   13
```

```r
MergedDataTables[FiscalYear & June, Special.Notes]
```
...
Results of library(quantmod)
        ...
        The following object is masked from 'package:data.table':  last
...
Command Output
        ...
        downloaded length 104011 != reported length 200
```
...
Results
  ...
  2014     44     42       46      46        46  224
  Sum     397    374      401     406       408 1986
```
```
markdown love :-)
```